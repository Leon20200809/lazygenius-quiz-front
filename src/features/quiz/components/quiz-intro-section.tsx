 ...
 * クイズ開始前のイントロ画面を表示する
 *
 * アプリの概要・作成理由・使用技術を表示し、
 * STARTボタンが押されたことを親コンポーネントへ通知する。
 *
 * @param onStartQuiz クイズ開始処理
 */
export function QuizIntroSection({ onStartQuiz }: QuizIntroSectionProps) {
  return (
    <section className="mx-auto max-w-4xl px-6 py-16">
      ...
      <button
        type="button"
        onClick={onStartQuiz}
        className="cursor-pointer rounded-full bg-slate-950 px-8 py-3 text-sm font-bold text-white transition-transform hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2 active:scale-95"
      >
        START
      </button>
